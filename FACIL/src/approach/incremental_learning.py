...
class Inc_Learning_Appr:
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 eval_on_train=False, logger: ExperimentLogger = None, exemplars_dataset: ExemplarsDataset = None, data_type='TSN'):
        self.model = model
        self.device = device
    ...
    def _get_scheduler(self, optimizer, nepochs=50):
        """Returns the scheduler"""
        if nepochs==60:
            return torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20,40], gamma=0.1, verbose=True)
        else:
            return torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20], gamma=0.1, verbose=True)
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        self.scheduler = self._get_scheduler(self.optimizer, self.nepochs)
    ...
    def train_epoch(self, t, trn_loader):
        """Runs a single epoch"""
        self.model.train()
        # self.model.model.num_segments = 5
    ...
    def eval(self, t, val_loader, save_outputs=False):
        """Contains the evaluation code"""
        with torch.no_grad():
            total_loss, total_acc_taw, total_acc_tag, total_num = 0, 0, 0, 0
            self.model.eval()
            # self.model.model.num_segments=10
            # print(self.model.num_segments)
            if save_outputs:
                all_outputs = []
                all_targets = []
            for images, targets in val_loader:
                ...
                if save_outputs:
                    all_outputs.append(torch.cat(outputs, dim=1))
        ...
        if save_outputs:
            return total_loss / total_num, total_acc_taw / total_num, total_acc_tag / total_num, all_outputs, all_targets
        else:
            return total_loss / total_num, total_acc_taw / total_num, total_acc_tag / total_num

    def eval_met(self, outputs, targets):
        hits_taw, hits_tag = self.calculate_metrics_tag(outputs, targets)
        total_loss, total_acc_taw, total_acc_tag, total_num = 0, 0, 0, 0
        # Log
        total_acc_taw += hits_taw.sum().item()
        total_acc_tag += hits_tag.sum().item()
        ...
        return total_acc_taw / total_num, total_acc_tag / total_num

    def calculate_metrics_tag(self, outputs, targets):
