...
class ExemplarsDatasetTSN(TSNDataSet):
    """Exemplar storage for approaches with an interface of Dataset"""

    def __init__(self, transform, class_indices, num_segments=5, new_length=1, modality='RGB',
                 image_tmpl='img_{:05d}.jpg', num_exemplars=0, num_exemplars_per_class=0, exemplar_selection='random'):
        super().__init__({'x': [], 'y': []}, transform=transform, class_indices=class_indices, num_segments=num_segments, new_length=new_length, modality=modality, image_tmpl=image_tmpl)
        self.max_num_exemplars_per_class = num_exemplars_per_class
        self.max_num_exemplars = num_exemplars
        assert (num_exemplars_per_class == 0) or (num_exemplars == 0), 'Cannot use both limits at once!'
        cls_name = "{}ExemplarsSelector".format(exemplar_selection.capitalize())
        selector_cls = getattr(importlib.import_module(name='datasets.exemplars_selection_tsn'), cls_name)
        self.exemplars_selector = selector_cls(self)
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            self.paths, self.labels = self.exemplars_selector(model, trn_loader, selection_transform)
