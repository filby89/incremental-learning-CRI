...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform):
        ...
        #     x, y = zip(*(idx for idx in selected_indices))
            if isinstance(ds_for_selection,ConcatDataset):
                all_paths = []
                all_labels = []
                for ds in ds_for_selection.datasets:
                    all_paths.extend(ds.paths)
                    all_labels.extend(ds.labels)
            else:
                all_paths = ds_for_selection.paths
                all_labels = ds_for_selection.labels

            x = [all_paths[idx] for idx in selected_indices]
            y = [all_labels[idx] for idx in selected_indices]
    ...
    def _exemplars_per_class_num(self, model: LLL_Net):
        ...
        exemplars_per_class = int(np.floor(num_exemplars / num_cls)) # change to floor
