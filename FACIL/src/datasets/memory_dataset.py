...
def get_data(trn_data, val_data, tst_data, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    # ALL OR TEST
    filtering = np.isin(val_data['y'], class_order)
    if filtering.sum() != len(val_data['y']):
        val_data['x'] = val_data['x'][filtering]
        val_data['y'] = val_data['y'][filtering]
    for this_image, this_label in zip(val_data['x'], val_data['y']):
        # If shuffling is false, it won't change the class number
        this_label = class_order.index(this_label)
        # add it to the corresponding split
        this_task = (this_label >= cpertask_cumsum).sum()
