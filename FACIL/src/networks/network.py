...
class LLL_Net(nn.Module):
    ...
    def forward(self, x, return_features=False):
        """Applies the forward pass with consensus for TSNs
        ...
        for head in self.heads:
            base_out = head(x)
            base_out = base_out.view((-1, self.model.num_segments) + base_out.size()[1:])
            output = self.model.consensus(base_out).squeeze(1)

            y.append(output)
        if return_features:
            x = x.view((-1, self.model.num_segments) + x.size()[1:])
            x = self.model.consensus(x).squeeze(1)
